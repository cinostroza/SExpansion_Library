...
public class III_Signature_Sem_ord4 {
	...
	public static void main(String[] args) {
		...
		// TODO Auto-generated method stub
		Semigroup [] listaSemigrupos = Semigroup.loadFromFile() ;
		...
		Semigroup semigroup ;
		int i;
		int [] quants = new int[40];
		int [][] eigens = new int[40][3];
		int nPos = 0 ;
		int pos = 0 ;
	    ...
	    for ( i = 0 ; i < listaSemigrupos.length ; ++i) {
	    	...
	    	if ( semigroup.order == 4 && semigroup.isCommutative()) {
	    		nCommutativos++;
	    		...
	    		int [] r = III_Signature_Sem_ord3.classifyEigenvalues(eigenvalueMatrix);
	    		for ( int j = 0 ; j < r.length ; ++j){
	    			System.out.println(r[j]);
	    		}
	    		//If we do not have the element we add it
	    		pos = III_Signature_Sem_ord3.findInList(eigens, r, nPos);
	    		if (  pos == -1 ) {
	    ...
	    System.out.println(nCommutativos);
	    System.out.println("commutatives");
	    for ( j = 0 ; j < nPos ; ++j) {
	    	System.out.println("****");
