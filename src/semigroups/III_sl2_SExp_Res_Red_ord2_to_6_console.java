...
public class III_sl2_SExp_Res_Red_ord2_to_6_console {
	...
	public static void main(String[] args) {
		...
		Semigroup [] listSemigroups = Semigroup.loadFromFile() ;
		...
		System.out.println("zero element and a resonance; and identify the ones that leads to a semisimple algebra.");
		System.out.println("**********");
		System.out.println("");
		Semigroup semigroup;
	    SetS [][] resonances ;
	    ...
	    StructureConstantSetExpandedResonantReduced ResRedAlgebra  = null; 
	    int nResRed = 0 , nSemisimple = 0  ;
	    ...
	    for ( i = 0 ; i < listSemigroups.length ; ++i) {
	    	...
	    	if ( semigroup.order == order && semigroup.isCommutative()) {
	    		...
	    		if ( resonances != null && semigroup.findZero() != -1 ) {
	    			nResRed = nResRed + 1 ;
	    			AllSemisimple = true ;
	    			for ( j = 0 ; j < resonances.length ; ++j ) {
	    				...
	    				ResRedAlgebra = new StructureConstantSetExpandedResonantReduced( ResAlgebra , semigroup.findZero() ) ;
	    				metric = ResRedAlgebra.cartanKillingMetricPretty() ;
	    				if ( metric.det() != 0.0 ) {
	    					...
	    					semigroup.show();
	    					System.out.println("which has the zero element "+semigroup.findZero()+" and the resonance:");
	    					System.out.print("S0: ");
			    			S0.show();
			    			System.out.print("S1: " ) ;
			    			S1.show();
	    ...
	    System.out.print(nResRed);
	    System.out.println(" semigroups of order "+order+" with zero element and at least one resonant decomposition.");
	    ...
	    System.out.println( " semigroups with zero element, having more than one resonance and");
